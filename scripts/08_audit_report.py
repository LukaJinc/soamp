...
def main():
    ...
    md.append("")
    md.append("- QMAP dataset: one peptide (id=21052) has a leading space in its raw DBAASP "
              "`sequence` field (`' LLLRRRRLL'`) -- a source data artifact, not corrected here.")
    ...
    md.append("- If `qmap.toolkit.train_test_split` failed for any reason at Step 6 run time, "
              "that is called out explicitly in the Step 6 log/section above rather than silently "
              "shipping an unsplit or improperly split dataset.")
    md.append("")
